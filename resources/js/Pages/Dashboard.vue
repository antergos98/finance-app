<template>
    <AppLayout>
        <template v-slot:header>
            <EntriesHeader :totalBalance="page.props.totalBalance" />
        </template>

        <div v-if="hasEntries">
            <GroupedEntries
                v-for="(entries, date) in entries"
                :key="date"
                :entries="entries"
            />
        </div>
        <div v-else class="text-center">
            ...
            <h2 class="text-2xl font-bold mt-8 text-gray-600 mb-5">
                No entries yet.
            </h2>
            <ModalLink
                class="px-10 py-4 transition bg-blue-600 hover:bg-blue-700 hover:shadow-sm rounded-md text-white font-bold uppercase tracking-tight"
                href="/entries/create"
                :close-button="false"
                max-width="3xl"
            >
...
import EntriesHeader from "./Components/EntriesHeader.vue";
import GroupedEntries from "./Components/GroupedEntries.vue";
...
const hasEntries = computed(() => Object.keys(props.entries).length > 0);

Echo.private(`user.${page.props.user_id}`).listen("EntriesImportFinished", () =>
    router.reload(),
);
</script>
